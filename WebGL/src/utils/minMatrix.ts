...
const MatIV: {[key: string]: any} = {
  ...
  rotate: (mat: Float32Array, angle: number, axis: number[], dest: Float32Array): Float32Array => {
    ...
    if (!sq) { throw Error('"axis" is invalid.'); }
  ...
  lookAt: (eye: number[], center: number[], up: number[], dest: Float32Array): Float32Array => {
    ...
    const centerX = center[0]; const centerY = center[1]; const
      centerZ = center[2];
    if (eyeX === centerX && eyeY === centerY && eyeZ === centerZ) { return MatIV.identity(dest); }
  ...
  perspective: (
    fovy: number, aspect: number, near: number, far: number, dest: Float32Array,
  ): Float32Array => {
